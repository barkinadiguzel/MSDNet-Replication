...
class MSDNet(nn.Module):
    def __init__(self, in_channels=3, num_classes=100, num_scales=3, depth=4, channels=32, exit_layers=None, exit_thresholds=None):
        ...
        for s in range(num_scales):
            self.init_blocks.append(nn.Conv2d(in_channels, channels, 3, padding=1))
    ...
    def forward(self, x):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i in self.exit_layers:
                ...
                mask = max_conf >= self.exit_thresholds[idx]
                if mask.any():
                    out = logits
                    break  

        if out is None:
            out = self.classifiers[-1](features[-1])
        return out
