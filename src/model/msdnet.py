...
class MSDNetSampleLevel(nn.Module):
    def __init__(self, in_channels=3, num_classes=100, num_scales=3, depth=4, channels=32, exit_layers=None, exit_thresholds=None):
        ...
        self.blocks = nn.ModuleList([
            MultiScaleBlock([channels]*num_scales, [channels]*num_scales, [1]*num_scales) 
    ...
    def forward(self, x):
        ...
        for i, block in enumerate(self.blocks):
            mask = ~exited
            if mask.any():
                selected_x = [f[mask] for f in features]
                features_masked = block(selected_x)
                for idx, f in enumerate(features):
                    f[mask] = features_masked[idx]
            ...
            if i in self.exit_layers:
                ...
                exit_mask = (max_conf >= self.exit_thresholds[idx]) & (~exited)
                outputs[exit_mask] = logits[exit_mask]
                exited[exit_mask] = True
        ...
        if remaining.any():
            ...
            outputs[remaining] = final_logits[remaining]
